...
/// Google Sign-In method type
enum GoogleSignInMethod {
  oneTap,  // Google One Tap (corner popup)
  popup,   // Traditional popup window
}
...
class GoogleSignInService {
  ...
  /// Smart sign in - tries One Tap first, falls back to popup
  Future<GoogleSignInAccount?> signIn() async {
    try {
      print('🔵 Trying Google One Tap first...');
      return await signInWithOneTap();
    } catch (e) {
      print('⚠️ One Tap failed/unavailable, falling back to popup: $e');
      AppLogger.info('One Tap unavailable, using popup fallback');
      return await signInWithPopup();
    }
  }

  /// Sign in with Google One Tap (corner popup)
  Future<GoogleSignInAccount?> signInWithOneTap() async {
    try {
      print('🔵 Google One Tap: Starting...');
      AppLogger.info('🔵 Google One Tap: Starting...');
      ...
      // Show One Tap prompt
      print('🔵 Showing One Tap prompt...');
      id.callMethod('prompt', [
        js.allowInterop((notification) {
          print('🔵 One Tap notification: ${notification}');
          // Check if One Tap was not displayed or skipped
          try {
            ...
            if (isNotDisplayed == true || isSkippedMoment == true) {
              print('⚠️ One Tap not displayed or skipped');
              if (_signInCompleter != null && !_signInCompleter!.isCompleted) {
                _signInCompleter!.completeError(
                  AuthException('One Tap not available'),
                );
              }
            }
          } catch (e) {
            print('⚠️ Error checking One Tap notification: $e');
          }
        })
      ]);

      // Wait for the callback (shorter timeout for One Tap)
      print('⏳ Waiting for One Tap callback (30s timeout)...');
      final account = await _signInCompleter!.future.timeout(
        const Duration(seconds: 30),
        onTimeout: () {
          print('❌ TIMEOUT: One Tap did not complete');
          throw AuthException('One Tap timeout');
        },
      );

      print('✅ Google One Tap successful!');
      AppLogger.info('✅ Google One Tap successful!');
      return account;
    } catch (e) {
      print('❌ Google One Tap failed: $e');
      print('❌ Error type: ${e.runtimeType}');
      AppLogger.error('❌ Google One Tap failed: $e');
      rethrow; // Rethrow to allow fallback
    }
  }

  /// Sign in with popup window (traditional method)
  Future<GoogleSignInAccount?> signInWithPopup() async {
    try {
      print('🔵 Google Popup Sign-In: Starting...');
      AppLogger.info('🔵 Google Popup Sign-In: Starting...');
      ...
      AppLogger.info('🔵 Opening Google Sign-In popup window...');
      ...
      final redirectUri = Uri.encodeComponent('http://localhost:3000');
      ...
      subscription = html.window.onMessage.listen((event) {
        ...
        AppLogger.info('🔵 Received message from popup: ${event.data}');
          ...
          if (event.data != null) {
            ...
            if (data is Map) {
              if (data['type'] == 'google-signin-success') {
                ...
                AppLogger.info('🔵 Extracted ID token from message');
              }
            }
            ...
            if (idToken != null && idToken.isNotEmpty) {
              ...
              if (_signInCompleter != null && !_signInCompleter!.isCompleted) {
                AppLogger.info('✅ Completing sign-in with token');
