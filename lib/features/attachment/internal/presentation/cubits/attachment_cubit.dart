...
class AttachmentCubit extends Cubit<AttachmentState> {
  ...
  List<PendingAttachment> get pendingAttachments => List.unmodifiable(_pendingAttachments);

  /// Add a pending attachment (not uploaded yet)
  void addPending(PendingAttachment attachment) {
    _pendingAttachments.add(attachment);
    emit(AttachmentState.uploaded(urls: _uploadedUrls, pending: _pendingAttachments));
  }
  ...
  Future<void> uploadPendingFiles({required String featureName, String? userId}) async {
    if (_pendingAttachments.isEmpty) return;

    final filePaths = _pendingAttachments.where((a) => a.filePath != null).map((a) => a.filePath!).toList();
    ...
    try {
      ...
      final results = await uploadBatchFilesUseCase(
        ...
        onProgress: (current, total) {
          emit(AttachmentState.uploading(currentFile: current, totalFiles: total, progress: current / total));
        },
      );
