...
class AttachmentPickerWidget extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AttachmentPickerWidget> createState() => _AttachmentPickerWidgetState();
}
            ...
            child: isImage
                ...
                : Center(
                    child: Column(
                      ...
                      children: [
                        Icon(_getFileIcon(url), size: 32, color: theme.colorScheme.primary),
                        const SizedBox(height: 4),
                        Text(_getFileExtension(url).toUpperCase(), style: theme.textTheme.labelSmall),
                      ],
                    ),
                  ),
          ),
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('✅ File uploaded successfully'), duration: Duration(seconds: 2)));
      }
  ...
  Future<void> _removeAttachment(int index) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('✅ File deleted successfully'), duration: Duration(seconds: 2)));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
