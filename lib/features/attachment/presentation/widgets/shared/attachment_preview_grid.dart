...
class AttachmentPreviewGrid extends StatelessWidget {
  const AttachmentPreviewGrid({required this.attachments, this.onRemove, this.readOnly = false, super.key});
  ...
  Widget build(BuildContext context) {
    AppLogger.debug('📊 Building preview grid with ${attachments.length} attachments', tag: 'AttachmentPreview');
    for (var i = 0; i < attachments.length; i++) {
      final item = attachments[i];
      AppLogger.debug('  [$i] isUploaded: ${item.isUploaded}, url: ${item.uploadedUrl}', tag: 'AttachmentPreview');
    }
  ...
  void _viewFullImage(BuildContext context, AttachmentItem item) {
    AppLogger.debug('🖼️ Attempting to view full image', tag: 'AttachmentPreview');
    AppLogger.debug('📋 Item isUploaded: ${item.isUploaded}', tag: 'AttachmentPreview');
    AppLogger.debug('🔗 Item uploadedUrl: ${item.uploadedUrl}', tag: 'AttachmentPreview');

    if (item.uploadedUrl == null) {
      AppLogger.error('⚠️ Cannot view image - uploadedUrl is null', tag: 'AttachmentPreview');
      return;
    }

    AppLogger.info('✅ Opening image dialog for URL: ${item.uploadedUrl}', tag: 'AttachmentPreview');
