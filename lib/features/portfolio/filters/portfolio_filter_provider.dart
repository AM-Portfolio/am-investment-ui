...
class PortfolioFilterProvider implements FilterProvider<dynamic> {
  ...
  FilterOptions extractFilterOptions(List<dynamic> items) {
    ...
    final marketCaps = <String>{'Small Cap', 'Mid Cap', 'Large Cap', 'Mega Cap'};
    ...
    for (final item in items) {
      ...
      if (totalValue > 0) {
        minValue = minValue == double.infinity ? totalValue : minValue.compareTo(totalValue) < 0 ? minValue : totalValue;
        maxValue = maxValue == double.negativeInfinity ? totalValue : maxValue.compareTo(totalValue) > 0 ? maxValue : totalValue;
      }
    }
  ...
  String _classifyMarketCap(double marketCap) {
    if (marketCap >= 200000000000) { // 200B+
      return 'Mega Cap';
    } else if (marketCap >= 10000000000) { // 10B - 200B
      return 'Large Cap';
