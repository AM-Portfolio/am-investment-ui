...
class WatchlistFilterProvider implements FilterProvider<dynamic> {
  ...
  FilterOptions extractFilterOptions(List<dynamic> items) {
    ...
    double minMarketCap = double.infinity;
    double maxMarketCap = double.negativeInfinity;
    ...
    for (final item in items) {
      ...
      if (currentPrice > 0) {
        minPrice = minPrice == double.infinity ? currentPrice : minPrice.compareTo(currentPrice) < 0 ? minPrice : currentPrice;
        maxPrice = maxPrice == double.negativeInfinity ? currentPrice : maxPrice.compareTo(currentPrice) > 0 ? maxPrice : currentPrice;
      }
      
      final marketCap = _getDoubleValue(item, 'marketCap');
      if (marketCap > 0) {
        minMarketCap = minMarketCap == double.infinity ? marketCap : minMarketCap.compareTo(marketCap) < 0 ? minMarketCap : marketCap;
        maxMarketCap = maxMarketCap == double.negativeInfinity ? marketCap : maxMarketCap.compareTo(marketCap) > 0 ? maxMarketCap : marketCap;
      }
      
      final volume = _getDoubleValue(item, 'volume');
      if (volume > 0) {
        minVolume = minVolume == double.infinity ? volume : minVolume.compareTo(volume) < 0 ? minVolume : volume;
        maxVolume = maxVolume == double.negativeInfinity ? volume : maxVolume.compareTo(volume) > 0 ? maxVolume : volume;
      }
    }
  ...
  @override
  List<dynamic> applyFilters(List<dynamic> items, List<FilterCriteria> filters) {
  ...
  List<dynamic> _applyCategoryFilter(List<dynamic> items, FilterCriteria filter) {
    if (filter.selectedCategories == null || filter.selectedCategories!.isEmpty) {
      return items;
    }
  ...
  List<dynamic> _applyPerformanceFilter(List<dynamic> items, FilterCriteria filter) {
  ...
  String _getStringValue(dynamic item, String field) {
    try {
  ...
  double _getDoubleValue(dynamic item, String field) {
    try {
      if (item is Map<String, dynamic>) {
