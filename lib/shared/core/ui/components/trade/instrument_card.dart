      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.abc, size: 16, color: Colors.blue.shade700),
                const SizedBox(width: 8),
                Text('Instrument', style: theme.textTheme.labelLarge?.copyWith(fontWeight: FontWeight.bold)),
              ],
            ),
          ),
          Padding(
                  ...
                  items: ExchangeTypes.values
                      .map(
                        (exchange) => DropdownMenuItem(
                          value: exchange,
                          child: Text(exchange.toString().split('.').last.toUpperCase()),
                        ),
                      )
                      .toList(),
                  ...
                  items: MarketSegments.values
                      .map(
                        (segment) => DropdownMenuItem(
                          value: segment,
                          child: Text(segment.toString().split('.').last.toUpperCase()),
                        ),
                      )
                      .toList(),
