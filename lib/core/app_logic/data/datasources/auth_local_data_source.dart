...
class AuthLocalDataSource implements AuthDataSource {
  ...
  Future<void> _loadTestUsers() async {
    ...
    try {
      AppLogger.info('Loading test users from asset file', tag: 'AuthLocalDataSource');
      
      final String jsonString = await rootBundle.loadString(AssetPaths.testUsers);
      final Map<String, dynamic> jsonData = json.decode(jsonString);
      final List<dynamic> usersList = jsonData['users'];

      AppLogger.debug('Parsing ${usersList.length} test users', tag: 'AuthLocalDataSource');
      ...
      AppLogger.info('Successfully loaded ${usersList.length} test users', tag: 'AuthLocalDataSource');
      AppLogger.methodExit('_loadTestUsers', tag: 'AuthLocalDataSource', result: 'success');
    } catch (e, stackTrace) {
      AppLogger.error('Failed to load test users from ${AssetPaths.testUsers}', 
          tag: 'AuthLocalDataSource', error: e, stackTrace: stackTrace);
      AppLogger.methodExit('_loadTestUsers', tag: 'AuthLocalDataSource', result: 'error');
      
      // Throw meaningful error with context
      if (e.toString().contains('Unable to load asset')) {
        throw Exception('Failed to load test users: Asset file ${AssetPaths.testUsers} not found. '
            'Ensure the file exists and is properly listed in pubspec.yaml assets section.');
      } else if (e.toString().contains('FormatException')) {
        throw Exception('Failed to load test users: Invalid JSON format in ${AssetPaths.testUsers}. '
  ...
  Future<AuthDataResponse> login(String identifier, String password) async {
    AppLogger.methodEntry('login', tag: 'AuthLocalDataSource', 
    ...
    if (_testUsers.containsKey(identifier)) {
      ...
      if (testUser.password == password) {
        AppLogger.info('Test user authentication successful for user: ${testUser.email}', 
        ...
        AppLogger.methodExit('login', tag: 'AuthLocalDataSource', result: 'success');
        ...
        AppLogger.warning('Invalid password for test user', tag: 'AuthLocalDataSource');
        AppLogger.methodExit('login', tag: 'AuthLocalDataSource', result: 'invalid_password');
    ...
    if (identifier.contains('test') || identifier.contains('demo')) {
      AppLogger.info('Demo mode: Creating demo user for test identifier', tag: 'AuthLocalDataSource');
      ...
      AppLogger.methodExit('login', tag: 'AuthLocalDataSource', result: 'demo_success');
      return AuthDataResponse(
        user: user,
    ...
    AppLogger.warning('Invalid credentials for identifier', tag: 'AuthLocalDataSource');
    AppLogger.methodExit('login', tag: 'AuthLocalDataSource', result: 'invalid_credentials');
  ...
  Future<AuthDataResponse> register(
    ...
    AppLogger.methodEntry('register', tag: 'AuthLocalDataSource', 
        params: {'name': name, 'email': email, 'username': username, 'phone': phone});
    
    // For demo mode, always allow registration
    AppLogger.info('Demo mode: Creating demo user for registration', tag: 'AuthLocalDataSource');
    final user = UserMapper.fromJson({
    ...
    AppLogger.methodExit('register', tag: 'AuthLocalDataSource', result: 'success');
    return AuthDataResponse(
      user: user,
  ...
  Future<String> refreshToken(String token) async {
    ...
    final newToken = 'demo_token_refreshed_${DateTime.now().millisecondsSinceEpoch}';
    
    AppLogger.methodExit('refreshToken', tag: 'AuthLocalDataSource', result: 'success');
    return newToken;
  }

  @override
  Future<bool> isIdentifierTaken(String identifier, String type) async {
    AppLogger.methodEntry('isIdentifierTaken', tag: 'AuthLocalDataSource',
    ...
    AppLogger.methodExit('isIdentifierTaken', tag: 'AuthLocalDataSource', 
  ...
  Future<bool> changePassword(
    ...
    AppLogger.info('Demo mode: Password change simulated', tag: 'AuthLocalDataSource');
    
    AppLogger.methodExit('changePassword', tag: 'AuthLocalDataSource', result: 'success');
    return true;
  }

  @override
  Future<bool> requestPasswordReset(String email) async {
    AppLogger.methodEntry('requestPasswordReset', tag: 'AuthLocalDataSource',
        params: {'email': email});
    
    // For demo mode, always succeed
    AppLogger.info('Demo mode: Password reset request simulated', tag: 'AuthLocalDataSource');
    
    AppLogger.methodExit('requestPasswordReset', tag: 'AuthLocalDataSource', result: 'success');
    return true;
  }
  ...
  Future<List<User>> getTestUsers() async {
    ...
    AppLogger.methodExit('getTestUsers', tag: 'AuthLocalDataSource', 
