...
class AuthIntegrationTest {
  ...
  static Future<void> testCompleteFlow() async {
    if (!kDebugMode) {
      AppLogger.info(
        'Auth integration test skipped - not in debug mode',
        tag: 'AuthIntegrationTest',
      );
      return;
    }

    AppLogger.info(
      'Starting clean architecture auth integration test',
      tag: 'AuthIntegrationTest',
    );
    ...
    try {
      ...
      if (loginResult.isSuccess) {
        ...
        AppLogger.info(
          '✅ Current user retrieved: ${currentUser?.email}',
          tag: 'AuthIntegrationTest',
        );
        ...
        if (isLoggedOut) {
          ...
          AppLogger.warning(
            '❌ Logout test failed - user still authenticated',
            tag: 'AuthIntegrationTest',
          );
        }
      } else {
        AppLogger.warning(
          '❌ Login test failed: ${loginResult.errorMessage}',
          tag: 'AuthIntegrationTest',
        );
      }

      AppLogger.info(
        '🎉 Clean architecture auth integration test completed!',
        tag: 'AuthIntegrationTest',
      );
    } catch (e, stackTrace) {
      AppLogger.error(
