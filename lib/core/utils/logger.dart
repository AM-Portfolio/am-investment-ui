...
class AppLogger {
  ...
  static void initialize() {
    ...
    if (_loggingEnabled) {
      info('AppLogger initialized for environment: ${EnvironmentConfig.environment.name}', 
           tag: 'Logger');
    }
  }
  ...
  static void _log(String message, LogLevel level, {
    ...
    if (kDebugMode) {
      ...
      final coloredMessage = '${_getColorCode(level)}$formattedMessage$_resetColor';
  ...
  static void methodEntry(String methodName, {String? tag, Map<String, dynamic>? params}) {
    if (params != null && params.isNotEmpty) {
      debug('→ $methodName(${params.entries.map((e) => '${e.key}: ${e.value}').join(', ')})', tag: tag);
    } else {
