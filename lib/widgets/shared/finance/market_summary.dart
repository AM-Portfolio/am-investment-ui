  ...
  Widget build(BuildContext context) {
    ...
    marketData.sort((a, b) {
      ...
      final aChange = double.parse(
        (a['change'] as String).replaceAll('%', '').replaceAll('+', ''),
      );
      final bChange = double.parse(
        (b['change'] as String).replaceAll('%', '').replaceAll('+', ''),
      );
                  ...
                  ? Expanded(
                      child: TabBarView(
                        ...
                        children: [
                          ...
                          _buildMarketList(
                            theme,
                            marketCategories[0]['data'] as List,
                          ),

                          // Global Markets tab
                          _buildMarketList(
                            theme,
                            marketCategories[1]['data'] as List,
                          ),
                        ],
                      ),
                    )
...
class TrendLinePainter extends CustomPainter {
  ...
  /// Constructor
  const TrendLinePainter({required this.points, required this.color});
